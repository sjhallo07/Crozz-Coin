...
import { GrpcConnectionSelector, GrpcConnectionBadge } from './components/GrpcConnection';
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        <header className="bg-white shadow-sm sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-900">Sui gRPC Explorer</h1>
            <GrpcConnectionBadge />
          </div>
        </header>
        ...
        <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
          ...
          <section>
            <h2 className="text-xl font-bold text-gray-900 mb-4">Configuración de Conexión</h2>
            <GrpcConnectionSelector />
          </section>

          {/* Explorador de APIs */}
          <section>
            <h2 className="text-xl font-bold text-gray-900 mb-4">Explorador de APIs</h2>
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      ...
      <div className="space-y-2 text-sm">
        <p>
          Estado: <span className="font-semibold">{isConnected ? '✓ Conectado' : '✗ Desconectado'}</span>
        </p>
        <p>
          Ambiente: <span className="font-mono">{environment}</span>
        </p>
        <p>
          Endpoint: <span className="font-mono break-all">{currentEndpoint}</span>
        </p>
      </div>
    </div>
  );
