...
export const GrpcApiExplorer: React.FC<GrpcApiExplorerProps> = ({
  ...
  const { record: suiNSRecord, loading: suiNSLoading } =
    useSuiNSResolver(suiNSName);
  const { latestCheckpoint, loading: streamLoading } =
    useCheckpointSubscription();
    ...
    return (
      <Box style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          ...
          <Text color="gray">
            Por favor, configura la conexión gRPC primero en el selector de
            conexión.
          </Text>
        </Card>
      </Box>
