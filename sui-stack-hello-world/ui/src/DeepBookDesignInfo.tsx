...
export function DeepBookDesignInfo() {
  return (
    <Box mt="6" p="4" style={{ background: "var(--gray-a2)", borderRadius: 8 }}>
      ...
      <Box asChild>
        <ul style={{ margin: 0, paddingInlineStart: "18px" }}>
          <li><Text size="2">Per-pool taker/maker fees are bounded by pool type (e.g., volatile taker 1–10 bps, maker 0–5 bps; stable taker 0.1–1 bps, maker 0–0.5 bps; whitelisted 0).</Text></li>
          <li><Text size="2">Users stake DEEP in the pool to propose/vote; proposals and votes reset each epoch, queued to activate next epoch once quorum (≥50% voting power) is reached.</Text></li>
          <li><Text size="2">Voting power V uses cutoff Vc=100k DEEP: V = min(S, Vc) + max(√S − √Vc, 0).</Text></li>
        </ul>
      </Box>
      ...
      <Box asChild>
        <ol style={{ margin: 0, paddingInlineStart: "18px" }}>
          <li><Text size="2">Pool: <code>place_order_int</code> builds <em>OrderInfo</em>.</Text></li>
          <li><Text size="2">Book: validate → match against contra side (create fills) → inject remainder as on-book <em>Order</em>.</Text></li>
          <li><Text size="2">State: <code>process_create</code> applies fills, computes taker/maker fees (stake-gated discounts), updates account volumes/orders, returns settled/owed triples.</Text></li>
          <li><Text size="2">Vault: <code>settle_balance_manager</code> authorizes BalanceManager, nets base/quote/DEEP in/out and updates vault balances.</Text></li>
        </ol>
      </Box>
      ...
      <Box asChild>
        <ul style={{ margin: 0, paddingInlineStart: "18px" }}>
          <li><Text size="2">Epoch rollover moves current stats to history on first tx of the epoch.</Text></li>
          <li><Text size="2">Makers with stake ≥ required and volume in epoch can earn rebates; total rebates capped by DEEP fees for that epoch, excess DEEP is burned.</Text></li>
          <li><Text size="2">Account updates on first action each epoch to settle prior rebates and reset volumes.</Text></li>
        </ul>
      </Box>
