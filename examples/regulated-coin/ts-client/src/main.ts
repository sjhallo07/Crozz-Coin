...
const run = async () => {
    ...
    program
        ...
        .action((options) => {
            ...
            if (!DENY_CAP_ID)
                throw new Error(
                    "DENY_CAP_ID environment variable is not set. Are you sure the active address owns the deny list object?",
                );
    ...
    program
        ...
        .action((options) => {
            ...
            if (!TREASURY_CAP_ID)
                throw new Error("TREASURY_CAP_ID environment variable is not set.");

            const txb = new Transaction();

            const coin = txb.moveCall({
                target: `0x2::coin::mint`,
                arguments: [txb.object(TREASURY_CAP_ID), txb.pure.u64(options.amount)],
                typeArguments: [COIN_TYPE],
            });
    ...
    program
        ...
        .action((options) => {
            ...
            txb.moveCall({
                ...
                arguments: [txb.object(TREASURY_CAP_ID), txb.object(options.coin)],
                typeArguments: [COIN_TYPE],
            });
